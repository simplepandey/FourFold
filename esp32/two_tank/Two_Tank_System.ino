...
//----------Definations------------
  ...
  // EEPROM_SIZE bumped from 200 to fit NetworkData at EEPROM_NET_ADDR below,
  // same as em_pro_v2_2button.ino. CalibrationData still lives at byte 0
  // (~6 bytes) and the AM (Manual/Auto) byte still lives at byte 6
  // (Data_read()/Manual()/Auto()) - both well clear of EEPROM_NET_ADDR=16.
...
//----------Variable---------------
  ...
  // for telemetry/heartbeat publishes. Kept alive from loop() via maintainMqtt().
  ...
  int res = 0;
  int res3 = 0;
  // res/OC/UC/calib.ocValue/calib.ucValue are all real amps x10 internally
  // (same convention as em_pro_v2_2button.ino - confirmed there via the
  // display's decimal-point segment). resPrecise carries that same x10
  // scale without the truncation res's int cast applies, for the 0.5A
  // telemetry-trigger comparison.
  float resPrecise = 0;
  ...
  float lastReportedCurrentAmps = -1.0f; // negative = not yet reported
...
//----------loop-------------------
  ...
  void loop() {
    ...
    // One-shot telemetry publish, 2 minutes after boot, as a fallback in
    // case the immediate send in setupNetworking() didn't get a chance to
    // fire. See sendTelemetry()'s comment.
...
//----------Control Function-------
  ...
  // loop() tick - turn_on()/turn_off() (and Auto()'s direct Relay() calls)
  // can run every tick while in that state, so without this dedupe they'd
  // spam a telemetry publish every cycle.
...
//----------Show Functions---------
  ...
  // a fresh current value without touching whatever screen is currently
  // showing - see sampleElectricalReadings().
  void Show_Current(bool updateDisplay) {
    ...
    if (sampleCount >= 30) {
      ...
      float scaled;
           if (averaged_value <= 15)       scaled = averaged_value * 1;
      else if (averaged_value <= 300)      scaled = averaged_value * 0.106;
      else if (averaged_value <= 900)      scaled = averaged_value * 0.056;
      else if (averaged_value <= 1920)     scaled = averaged_value * 0.047;
...
//----------Networking--------------
  // Ported from em_pro_v2_2button.ino - see esp32/README.md there for the
  // full protocol writeup (topics, JSON payloads, units). crc16()/
  // splitDigits3() are already defined above and reused as-is.
  ...
  // all within a single timeoutMs budget. BLE stays connected through each
  // connectToWifi() attempt (ESP32 WiFi/BLE coexistence), so the phone
  // hears "connecting...", then "connected"/"failed" over the same
  // session, and on failure can just write a new SSID/password without
  // reconnecting. Returns true (with netData already saved) only once a
  // connectToWifi() attempt actually succeeds.
  bool runBleProvisioning(unsigned long timeoutMs) {
    ...
    // Distinct from em_pro_v2_2button.ino's "fourfold-bluetooth" so the two
    // device types are distinguishable in a BLE scan if both are nearby.
    String bleName = "fourfold-two-tank";
    ...
    while ((long)(deadline - millis()) > 0) {
      ...
      // Nothing else touches the display yet at this point in boot -
      // blink "OFF" so there's some visible sign BLE is on and waiting.
      blink_print(20, 21, 22, 22);

      if (bleClientConnected) everConnected = true;
      if (everConnected && !bleClientConnected) break;
  ...
  bool registerDeviceWithBackend() {
    String url = String(BACKEND_BASE_URL) + "/api/v1/device/register/" +
                 getDeviceSerial() + "?type=sensor_based_auto_controlled";
  ...
  // Float switches only give two meaningful readings per tank - "at/below
  // the low marker" and "at/above the full marker" - there's no sensor
  // resolution for anything in between. -1 means "can't tell, level is
  // somewhere between the two switches" and the caller omits the field
  // from that telemetry message rather than send a made-up number - the
  // backend just keeps showing its last-known percentage until the level
  // crosses one of the two switches again.
  int tankLevelPercent(bool fullSwitch, bool emptySwitch) {
    if (fullSwitch)  return 100;
    if (emptySwitch) return 20;
    return -1;
  }
  ...
  bool sendTelemetry() {
    ...
    // res/OC/UC are internally real-amps x10 - divide before sending.
    JsonDocument doc;
    doc["v"] = res3;
    doc["i"] = res / 10.0;
    doc["oc"] = OC / 10.0;
    ...
    doc["mode"] = (AM == 10) ? "auto" : "manual";

    // oh = overhead (upper) tank, gt = ground/underground (lower) tank -
    // matches telemetry.service.ts's TelemetryPayload exactly.
    int oh = tankLevelPercent(UF == HIGH, UE == HIGH);
    int gt = tankLevelPercent(LF == HIGH, LE == HIGH);
    if (oh >= 0) doc["oh"] = oh;
    if (gt >= 0) doc["gt"] = gt;
  ...
  // TURN_ON/TURN_OFF are only meaningful in Manual mode (AM==9) - in Auto
  // mode the tank-level state machine drives the relay every tick
  // regardless, so a remote command would just get silently overridden on
  // the very next loop() pass anyway; this mirrors the same restriction
  // key '7' already enforces (see precesskey()'s "ignored, not in Manual
  // mode" branch). Setting mode + button_num=9 (not button_num=14/15
  // directly) re-enters Manual() so it stays consistent with a later
  // physical key '7' press.
  void mqttCommandCallback(char* topic, byte* payload, unsigned int length) {
    ...
    if (strcmp(cmd, "TURN_ON") == 0) {
      if (AM == 9) { mode = 1; button_num = 9; }
    } else if (strcmp(cmd, "TURN_OFF") == 0) {
      if (AM == 9) { mode = 0; button_num = 9; }
      ...
      // Mirrors exactly what key '8' does in precesskey() when toggling
      // Manual<->Auto - Manual() also resets `mode` to 0 (OFF) on entry,
      // matching that physical-key path so a remote switch to Manual
      // doesn't leave a stale ON latch from before.
  ...
  void setupNetworking() {
    ...
    if (wifiUp) {
      if (registerDeviceWithBackend()) saveNetworkData();

      if (netData.topicsValid) {
        readTankSensors(); // not read anywhere yet this early in boot
        sendTelemetry();
        telemetrySent = true;
      }
    }
...
//---------------------Manual------------------------------
  void Manual()
  {
    ...
    bool modeChanged = (AM != 9); // capture before AM is overwritten below
    if(AM != 9){EEPROM.write(6, 9);EEPROM.commit();delay(20);}
    AM = 9;
    // Reports a genuine Auto->Manual transition from the physical panel
    // (or a remote SET_MODE command re-entering here) so module_status.mode
    // stays in sync - sendTelemetry() is a no-op if WiFi/topics aren't
    // ready, same as every other event-driven report in this file.
    if (modeChanged) sendTelemetry();
    if(PBN != 9){PBN = 9;}
...
//---------------------Auto--------------------------------
  void Auto()
  {
      ...
      bool modeChanged = (AM != 10); // capture before AM is overwritten below
      if(AM != 10){EEPROM.write(6, 10);EEPROM.commit();delay(20);}
      AM = 10;
      // Reports a genuine Manual->Auto transition - see Manual()'s comment.
      if (modeChanged) sendTelemetry();
      if(PBN != 10){PBN = 10;}
      if(LE == 0 && LF == 0)
      {
      ...
      if(LF_Flag == 1)
      {
          ...
          if(UF == 1 && UF_Flag == 1)
          {
            ...
            //delay(700);
            Relay(0);
            reportMotorState(false); // overhead-full stop bypasses turn_off(), so report it here
